...
<div class="bg-gray-700 max-w-3/10 min-w-xs w-full rounded-sm shadow-lg shadow-gray-850">
    <form action="{{ route('signSubmit') }}" method="post" style="max-height: 140rem;">
        @csrf
        <legend class="font-bold text-purple-600 text-4xl ml-5 mt-8 underline decoration-1 decoration-purple-400" style="text-shadow: 0px 2px 6px rgb(255,0,255, .5);">SignIn</legend>

        <div class="relative w-7/10 ml-4 mt-12">
            <input type="text" name="username" id="floating_outlined3" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-md border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-purple-400 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
            <label for="floating_outlined3" class="absolute text-sm text-gray-300 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] px-2 peer-focus:px-2 peer-focus:text-purple-300 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-85 peer-focus:-translate-y-8 rtl:peer-focus:translate-x-2/4 rtl:peer-focus:left-auto">USERNAME</label>
            @error('username')
